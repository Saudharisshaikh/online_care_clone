...
import com.app.onlinecare_pk.adapter.MyAppointmentsAdapter;
import com.app.onlinecare_pk.api.ApiManager;
import com.app.onlinecare_pk.b_health2.ActivityApptmntWaitingRoom;
import com.app.onlinecare_pk.model.MyAppointmentsModel;
...
public class MyAppointments extends BaseActivity {
	...
	public static MyAppointmentsModel myAppointmentsModelSelected;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lvMyAppointments.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				myAppointmentsModelSelected = DATA.allAppointments.get(position);

				if(myAppointmentsModelSelected.status.equalsIgnoreCase("Confirmed") && myAppointmentsModelSelected.isFiveMinuteFromNow){

					openActivity.open(ActivityApptmntWaitingRoom.class, false);
					...
					try {
						...
						if(status.equals("success")) {
							...
							temp = null;*/
									...
									String new_appointment_time = appointmentsArray.getJSONObject(i).getString("new_appointment_time");
									boolean isFiveMinuteFromNow = false;

									try {
										Date aptDateTime = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss", Locale.ENGLISH).parse(new_appointment_time);
										Date nowDateTime = new Date();
										long diff = aptDateTime.getTime() - nowDateTime.getTime();
										double numOfDays = (int) (diff / (1000 * 60 * 60 * 24));
										double hours = (int) (diff / (1000 * 60 * 60));
										double minutes = (int) (diff / (1000 * 60));
										double seconds = (int) (diff / (1000));
										DATA.print("-- time diffrence : "+ numOfDays+" Days, "+hours+" Hours, "+minutes+" Minutes, "+seconds+" Seconds");
										isFiveMinuteFromNow = minutes < 5;
									}catch (Exception e){
										e.printStackTrace();
									}
									...
									temp = new MyAppointmentsModel(appointment_id, appointment_date, first_name, last_name,
											...
											date,slot_id,reason,doctor_category, isFiveMinuteFromNow);
									DATA.allAppointments.add(temp);
									temp = null;
								}
