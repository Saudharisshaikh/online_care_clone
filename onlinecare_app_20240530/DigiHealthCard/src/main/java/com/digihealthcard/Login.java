...
import com.digihealthcard.util.GPSTracker;
...
public class Login extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, ApiCallBack{
	...
	Button btnLoginFB,btnLoginGoogle,ivLoginTwitter;//Button //change to button or ImageView only if change ui
	...
	public static String fullNameUserVerify , emailUserVerify , socialIdUserVerify , socialFromUserVerify;
...
//							}
			...
			try {
				...
				if(status.equals("success")) {
					...
					System.out.println("-- userStatus:: " + userVerifyStatus);
								...
								new GraphRequest.GraphJSONObjectCallback() {
									...
									public void onCompleted(JSONObject object, GraphResponse response) {
										...
										try{
											...
											fullNameUserVerify = fullName;
											emailUserVerify = email;
											socialIdUserVerify = id;
											socialFromUserVerify = "facebook";
											...
											params.put("first_name",fullName);
											params.put("hospital_id", HOSPITAL_ID_EMCURA);
											params.put("from_social", socialFromUserVerify);
											if (latitude == 0.0 || longitude==0.0) {
												gps = new GPSTracker(activity);
												if (gps.canGetLocation()) {
													latitude = gps.getLatitude();
