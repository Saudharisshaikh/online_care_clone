...
import com.app.OnlineCareTDC_Pt.adapter.LvLivecareDoctorsAdapter;
import com.app.OnlineCareTDC_Pt.adapter.NearbyHospitalsAdapter;
...
public class MapActivity extends AppCompatActivity implements GloabalSocket.SocketEmitterCallBack {
	...
	ExpandableHeightGridView gvDoctorsUrgentCare;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//hide lvDoctors and add gridview accoroding to TDC design - date 10-05-2022
		lvDoctors = (ListView) findViewById(R.id.lvDoctorsLivecare);
		etSearchDoc = findViewById(R.id.etSearchDoc);
		gvDoctorsUrgentCare = findViewById(R.id.gvDoctorsUrgentCare);
				...
				try{
					...
					try {
						...
						if (data.length() == 0) {
							...
							gvDoctorsUrgentCare.setAdapter(lvLivecareDoctorsAdapter);
				...
				try{
					...
					try {
						...
						if (jsonArray.length() == 0) {
							...
							gvDoctorsUrgentCare.setAdapter(adapter);
							if (new GloabalMethods(activity).checkPlayServices()) {
		...
		try {
			...
			if(usertype.equalsIgnoreCase("doctor")){
				if(status.equalsIgnoreCase("logout") && latLongBeansList != null){
					...
					if (connection.isConnectedToInternet()) {
						getNearByDrs(DATA.zipCodeFromLiveCare, false);
					} else {
						customToast.showToast(DATA.NO_NETWORK_MESSAGE,0,0);
					}
				}
