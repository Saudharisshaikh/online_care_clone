package com.app.msu_uc.b_health2;
...
import com.app.msu_uc.ActivityUrgentCareDoc;
import com.app.msu_uc.BaseActivity;
import com.app.msu_uc.Login;
import com.app.msu_uc.R;
import com.app.msu_uc.SearchADoctor;
import com.app.msu_uc.SelectReports;
import com.app.msu_uc.adapter.ConditionsAdapter;
import com.app.msu_uc.adapter.RvBodyPartAdapter;
import com.app.msu_uc.adapter.RvPainAdapter;
import com.app.msu_uc.adapter.VVReportImagesAdapter;
import com.app.msu_uc.api.ApiCallBack;
import com.app.msu_uc.api.ApiManager;
import com.app.msu_uc.b_health.assessment.ActivityDAST_Form;
import com.app.msu_uc.b_health.assessment.ActivityPHQ_Form;
import com.app.msu_uc.b_health.assessment.new_assesment.ActivityOCD_Form;
import com.app.msu_uc.b_health.assessment.new_assesment.ActivityStressQues_Form;
import com.app.msu_uc.model.ConditionsModel;
import com.app.msu_uc.model.SymptomsModel;
import com.app.msu_uc.paypal.PaymentLiveCare;
import com.app.msu_uc.util.ActionSheetPopup;
import com.app.msu_uc.util.CheckInternetConnection;
import com.app.msu_uc.util.CustomToast;
import com.app.msu_uc.util.DATA;
import com.app.msu_uc.util.Database;
import com.app.msu_uc.util.ExpandableHeightGridView;
import com.app.msu_uc.util.GloabalMethods;
import com.app.msu_uc.util.HideShowKeypad;
import com.app.msu_uc.util.LiveCareInsurance;
import com.app.msu_uc.util.OpenActivity;
import com.app.msu_uc.util.RecyclerItemClickListener;
...
public class GetLiveCareFormBhealth extends BaseActivity {
    ...
    static GetLiveCareFormBhealth activityGetLiveCareFormBhealth;
    ...
    TextView tvUrgentCareVisits, tvCounsling;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getSupportActionBar() != null){
            getSupportActionBar().setTitle("Emotional Wellness");//"Clinical Care"  Immediate Care
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            // getActionBar().setHomeButtonEnabled(true);
        }

        activityGetLiveCareFormBhealth = GetLiveCareFormBhealth.this;
                ...
                if(checkInternetConnection.isConnectedToInternet()) {
                    ...
                    /*else if(immeCareOrApptmnt == 0){
                        ...
                        if(!RvBodyPartAdapter.selectedPositons.isEmpty()){
                            String bodyPart = "";
                            for (int i = 0; i < GetLiveCare.bodyParts.length; i++) {
                                if(RvBodyPartAdapter.selectedPositons.contains(i)){
                                    bodyPart = bodyPart + GetLiveCare.bodyParts[i]+", ";
                                }
                            }
                            try {
                        ...
                        if(assessmentsAdded.isEmpty()){
                            ...
                            openActivity.open(ActivityConsent.class, false);//note : both options imme and apt are available in doc list
        ...
        gvSymptomNew.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                //Ahmer code 02-10-2023
                ArrayAdapter<ConditionsModel> conditionsAdapter = new ArrayAdapter<ConditionsModel>(activity, R.layout.listitem_singlechoice, android.R.id.text1, conditionsModels) {
                    @NonNull
                    @Override
                    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
                        View view = super.getView(position, convertView, parent);

                        // Find the CheckedTextView in the view
                        CheckedTextView textView = view.findViewById(android.R.id.text1);

                        // Set the text to the condition name
                        textView.setText(conditionsModels.get(position).conditionName);

                        // Get the existing left drawable from XML
                        Drawable leftDrawable = textView.getCompoundDrawables()[0];

                        // Check the condition and set the appropriate drawable resource
                        int drawableResource = 0;
                        if (conditionsModels.get(position).conditionName.equalsIgnoreCase("Anxiety Health")) {
                            drawableResource = R.drawable.ic_anxiety;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("Substance Abuse")) {
                            drawableResource = R.drawable.ic_substance_abuse;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("eating disorder")) {
                            drawableResource = R.drawable.ic_eating_disorder;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("stress")) {
                            drawableResource = R.drawable.ic_stress;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("ocd")) {
                            drawableResource = R.drawable.ic_ocd_form;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("depression analysis")) {
                            drawableResource = R.drawable.ic_depression;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("focus concentration")) {
                            drawableResource = R.drawable.ic_focus_conc;
                        } else if (conditionsModels.get(position).conditionName.equalsIgnoreCase("panic attack")) {
                            drawableResource = R.drawable.ic_panic_attack;
                        }

                        // Set the drawableLeft and drawableRight dynamically
                        textView.setCompoundDrawablesWithIntrinsicBounds(leftDrawable, getResources().getDrawable(drawableResource), null, null);
                        return view;
                    }
                };//Ahmer code

                //ArrayAdapter<ConditionsModel> conditionsAdapter = new ArrayAdapter<>(activity, R.layout.listitem_singlechoice_new, android.R.id.text1, conditionsModels);
		...
		gvConditionNew.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    ...
                    if(assessmentsAdded.isEmpty()){
                        btnLiveSbmtSymptom.setText("Submit");
                    }else {
                        btnLiveSbmtSymptom.setText("Next");
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
        ...
        rvBodyPart.addOnItemTouchListener(new RecyclerItemClickListener(activity, rvBodyPart, new RecyclerItemClickListener.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if(GetLiveCare.bodyParts[position].equalsIgnoreCase("Other")){
                    openKeyboardOnetExraInfo();
                }
            }
		...
		tvUrgentCareVisits = findViewById(R.id.tvUrgentCareVisits);
		tvCounsling = findViewById(R.id.tvCounsling);
        ...
        boolean isFromEmowellClick = getIntent().getBooleanExtra("isFromEmowellClick", false);
        int vis = isFromEmowellClick ? View.GONE : View.VISIBLE;
        tabOptions.setVisibility(vis);
    ...
    public void itrateAssessAdded(){

        if(assessmentIndex < assessmentsAdded.size()){
            String condName = assessmentsAdded.get(assessmentIndex);
