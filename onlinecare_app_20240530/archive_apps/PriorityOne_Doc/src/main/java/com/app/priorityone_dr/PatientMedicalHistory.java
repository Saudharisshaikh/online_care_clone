...
import com.app.priorityone_dr.adapter.HistoryMediAdapter;
import com.app.priorityone_dr.adapter.PastHistoryAdapter;
			...
			try {
				...
				//	medicalHistoryAdapter.notifyDataSetChanged();
					/*for (int i = 0; i < DATA.pastHistoryBeans.size(); i++) {
						if (DATA.selectedMedicalHistoryPositions.contains(DATA.pastHistoryBeans.get(i).getId())) {
							System.out.println("--in true bean id "+DATA.pastHistoryBeans.get(i).getId());
							DATA.pastHistoryBeans.get(i).setSelected(true);
						} else {
							System.out.println("--in false bean id "+DATA.pastHistoryBeans.get(i).getId());
					...
					/*for (Iterator<PastHistoryBean> it = DATA.pastHistoryBeans.iterator(); it.hasNext();) {
						if (! DATA.selectedMedicalHistoryPositions.contains(it.next().getId())) {
							//System.out.println("bean removed with id "+it.next().getId());
							it.next().setSelected(false);
							 it.remove();
						}else {

							 it.next().setSelected(true);
								System.out.println("bean added with id "+it.next().getId());
						}
						}*/
				...
				for (int i = DATA.pastHistoryBeans.size()-1; i >= 0; i--){
					...
					if (! DATA.selectedMedicalHistoryPositions.contains(DATA.pastHistoryBeans.get(i).getId())) {
						...
						DATA.pastHistoryBeans.get(i).setSelected(true);
						System.out.println("bean added with id "+DATA.pastHistoryBeans.get(i).getId());
					}
				}
