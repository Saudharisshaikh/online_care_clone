package com.onlinecare.broadcasts;
...
public class SaveGCM_TokenBroadcast extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals("com.onlinecare.covacard.SAVE_GCM_TOKEN")){
	...
	public void saveToken(String device_token) {
		...
		/*if (device_token.contains("|ID|")) {
			device_token = device_token.substring(device_token.indexOf(":") +1);
			 ...
			 System.out.println("--in params regId: "+device_token+" userid: "+ prefs.getString("id", "0")+" timezone "+getDeviceTimeZone());
			  ...
			  client.post(DATA.baseUrl+"/saveToken", params, new AsyncHttpResponseHandler() {
				  ...
				  public void onSuccess(int statusCode, Header[] headers, byte[] response) {
					  ...
					  try{
						  ...
						  // pd.dismiss();
						  System.out.println("--reaponce in saveDeiceId"+content);
					  }catch (Exception e){
						  e.printStackTrace();
						  System.out.println("-- responce onsuccess: saveDeiceId, http status code: "+statusCode+" Byte responce: "+response);
				  ...
				  public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
					  ...
					  try {
						  ...
						  System.out.println("--reaponce in onfailure saveDeiceId: "+content);
						  // pd.dismiss();
