package com.app.priorityone_uc.b_health.assessment;
...
import static com.app.priorityone_uc.MedicalHistory1.preventScrollViewFromScrollingToEdiText;
...
public class ActivityDAST_Form extends BaseActivity implements AssessSubmit{
	...
	public static final String DAST_FORM_TITTLE = "NDA Clinical Trials";
	...
	public void parseDASTFormData(String content){
		try {
			...
			String descTxt = "<b>"+form_title+"<br />General Instruction:</b><br />"+form_text;

			tvFormDesc.setText(Html.fromHtml(descTxt));//form_title+"\nGeneral Instruction:\n"+form_text
	...
	public void submitForm(String is_lock){
		...
		for (int i = 0; i < dasTfieldBeans.size(); i++) {
			System.out.println("ans["+i+"]"   + " = "  + dasTfieldBeans.get(i).scoreDerived);
		...
		if(apiName.equalsIgnoreCase(ApiManager.DAST_FORM)){
			...
			try {
				...
				if(jsonObject.getString("status").equalsIgnoreCase("success")){
					...
					GetLiveCareFormBhealth.dast_form = jsonObject.getString("id");

					ActivityDastList.shoulRefresh = true;
                    new GloabalMethods(activity).showAssesSavedDialog(totalScore);
