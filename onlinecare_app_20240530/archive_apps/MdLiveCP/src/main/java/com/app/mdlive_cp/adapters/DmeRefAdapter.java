...
import com.app.mdlive_cp.model.SoapReferralBean;
...
public class DmeRefAdapter extends ArrayAdapter<SoapReferralBean> {

	Activity activity;
	ArrayList<SoapReferralBean> soapReferralBeens;

	public DmeRefAdapter(Activity activity , ArrayList<SoapReferralBean> soapReferralBeens) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			...
			viewHolder.ivSoapRefSign = (ImageView) convertView.findViewById(R.id.ivSoapRefSign);
			...
			convertView.setTag(R.id.ivSoapRefSign,viewHolder.ivSoapRefSign);
		...
		if(!soapReferralBeens.get(position).dme_referral.isEmpty()){
			try {
				JSONObject jsonObject = new JSONObject(soapReferralBeens.get(position).dme_referral);
		...
		viewHolder.tvSoapRefStatus.setText("Status: "+soapReferralBeens.get(position).dme_status);

		if(signPath.isEmpty()){
			viewHolder.ivSoapRefSign.setVisibility(View.GONE);
		}else {
			viewHolder.ivSoapRefSign.setVisibility(View.VISIBLE);
			DATA.loadImageFromURL(signPath,0,viewHolder.ivSoapRefSign);
		}
		return convertView;
	}
