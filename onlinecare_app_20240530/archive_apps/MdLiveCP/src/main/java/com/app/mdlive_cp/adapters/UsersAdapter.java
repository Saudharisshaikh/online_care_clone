...
public class UsersAdapter extends ArrayAdapter<DoctorsModel> {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		/*if (DATA.allDoctors.get(position).speciality_id.equalsIgnoreCase("null")) {
			tvDocOrSp.setText("Doctor");
		} else {
			tvDocOrSp.setText("Specialist");
		}*/
		}else{
			System.out.println("-- DATA.allDoctors is null in UsersAdapter getview");
		}
	...
	public void filter(String filterText) {
		...
		if(filterText.length() == 0) {
			...
			for(DoctorsModel temp :doctorsModelsOrg) {
				...
				if(temp.fName.toLowerCase(Locale.getDefault()).contains(filterText) ||
