...
public class DrApointmentDetail extends BaseActivity {
	...
	Button btnApptCancel,btnCall,btnSendMsg,btnConfirmAppointment,btnCancelAppointment,btnSelPtViewReports;
	@Override
		...
		client.post(DATA.baseUrl+"/saveAppointmentStatus", params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: saveAppointmentStatus, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					...
					System.out.println("--onFailure in saveAppointmentStatus "+content);
