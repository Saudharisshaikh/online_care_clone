...
import com.app.mdlive_cp.adapters.DrAppointmentsAdapter;
import com.app.mdlive_cp.api.ApiManager;
import com.app.mdlive_cp.model.DrAppointmentModel;
import com.app.mdlive_cp.model.ReportsModel;
...
//					msg = jsonObject.getString("msg");
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: getDoctorAppointment, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					...
					System.out.println("-- on failure getDoctorAppointment: "+content);
	...
	public void refferToAdmin(ArrayList<String> appointment_ids) {
		...
		System.out.println("--params in refertoAdmin "+params.toString());
		...
		client.post(DATA.baseUrl+"/refertoAdmin/",params ,new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--responce in refertoAdmin: "+content);
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: refertoAdmin, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("--responce in failure refertoAdmin: "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
