...
import com.app.mdlive_cp.adapters.ProgressNotesAdapter;
import com.app.mdlive_cp.api.ApiManager;
import com.app.mdlive_cp.model.DoctorsModel;
import com.app.mdlive_cp.model.ProgressNoteBean;
...
public class ActivityProgressNotesView extends BaseActivity {
		...
		if(apiName.equalsIgnoreCase(ApiManager.GET_PROGRESS_NOTE)){
			try {
				...
				for (int i = 0; i < data.length(); i++) {
					...
					,feedback,ddate,ttime,session_length,first_name,last_name,patient_name,ot_data,symptom_name,condition_name,
							care_plan);
	...
	public void showAskDialog(){
		...
		dialogSupport.setContentView(R.layout.dialog_ask_pr_note);
		//dialogSupport.setCancelable(false);
		dialogSupport.setCanceledOnTouchOutside(false);

		Button btnNoteVitualVisit = dialogSupport.findViewById(R.id.btnNoteVitualVisit);
		Button btnNoteHomeVisit = dialogSupport.findViewById(R.id.btnNoteHomeVisit);


		btnNoteVitualVisit.setOnClickListener(new View.OnClickListener() {
		...
		btnNoteHomeVisit.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("isForHomeVisitNote",true);//Lisa added this fucntion to add note with real home visit(Patient home), it will not carry virtual visit data. 26feb2019
				startActivity(intent);
			}
		});
