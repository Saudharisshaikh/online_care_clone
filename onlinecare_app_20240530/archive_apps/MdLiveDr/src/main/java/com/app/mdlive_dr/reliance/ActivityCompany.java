package com.app.mdlive_dr.reliance;
...
import com.app.mdlive_dr.BaseActivity;
...
public class ActivityCompany extends BaseActivity implements  GloabalSocket.SocketEmitterCallBack{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lvCompanyPatients.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				DATA.selectedLiveCare.id = companyPatientBeans.get(position).patient_id;
				DATA.selectedLiveCare.is_online = companyPatientBeans.get(position).is_online;
				DATA.selectedLiveCare.first_name = companyPatientBeans.get(position).first_name;
				DATA.selectedLiveCare.last_name = companyPatientBeans.get(position).last_name;
				DATA.selectedLiveCare.patient_phone = companyPatientBeans.get(position).phone;
		...
		etFilter.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				if(companyPatientAdapter!=null){
					companyPatientAdapter.filter(s.toString());
				}
			}
		});
	...
	public void filterAndShowData(int isActive){//isActive-> 0=InActive, 1=Active
		...
		if(companyPatientBeansAll != null){
			...
			//show all patients active and inactive
			companyPatientBeans.addAll(companyPatientBeansAll);
		}
	...
	public void onSocketCallBack(String emitterResponse) {
		try {
			...
			if(usertype.equalsIgnoreCase("patient")){
				...
				if(DATA.selectedLiveCare != null && DATA.selectedLiveCare.id.equalsIgnoreCase(id)){
					if(status.equalsIgnoreCase("login")){
						DATA.selectedLiveCare.is_online = "1";
					}else if(status.equalsIgnoreCase("logout")){
						DATA.selectedLiveCare .is_online = "0";
					}
				}
			}
		}catch (Exception e){
