...
import com.app.mdlive_dr.adapter.CategoriesAdapter;
import com.app.mdlive_dr.api.ApiCallBack;
import com.app.mdlive_dr.api.ApiManager;
import com.app.mdlive_dr.model.CategoriesModel;
import com.app.mdlive_dr.permission.PermissionsActivity;
import com.app.mdlive_dr.permission.PermissionsChecker;
import com.app.mdlive_dr.reliance.ActivityCompany;
import com.app.mdlive_dr.reliance.therapist.ActivityReferedPatients;
import com.app.mdlive_dr.services.GPSTracker;
import com.app.mdlive_dr.util.ActionEditText;
import com.app.mdlive_dr.util.CallWebService;
...
import com.app.mdlive_dr.util.GloabalSocket;
...
public class MainActivityNew extends AppCompatActivity implements ApiCallBack, GloabalSocket.SocketEmitterCallBack{
	...
	TextView tvLiveCare, tvAppointments, tvDrProfile, tvLogout,tvDrSchedule,
			...
			tvInstantConnect,tvAppInvite,tvReffredPt,tvCallEncounters;
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if(tvRefillsBadge!=null){
					if(refills!= null){
						System.out.println("--refills size "+refills.size());
						if(refills.size() > 0){
				...
				if(tvAddDoctorBadge!=null){
					if(callInvites!= null){
						System.out.println("--callInvites size "+callInvites.size());
...
//			
//			System.out.println("--online care service started from MainActivityNew check...");
//
...
//		exportDB();
		...
		tvCallEncounters = findViewById(R.id.tvCallEncounters);
...
//				openActivity.open(HomeHealth.class, false);
		...
		tvCallEncounters.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				openActivity.open(ActivityCallLogsEncounters.class, false);
			}
		});
