...
import com.app.mdlive_dr.reliance.reschedule.ActivityDoctorSlots;
...
public class DrApointmentDetail extends BaseActivity {
	...
	Button btnApptCancel,btnCall,btnSendMsg,btnConfirmAppointment,btnCancelAppointment,btnSelPtViewReports,btnBackToHome,btnCancelAppointmentN,btnRescheduleAppointment,btnGoToPatientSheet;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnCancelAppointmentN = findViewById(R.id.btnCancelAppointmentN);
        btnRescheduleAppointment = findViewById(R.id.btnRescheduleAppointment);
		btnGoToPatientSheet = findViewById(R.id.btnGoToPatientSheet);
		...
		if (DATA.drAppointmentModel.getStatus().equalsIgnoreCase("Pending") || DATA.drAppointmentModel.getStatus().equalsIgnoreCase("Declined")) {
			...
			btnGoToPatientSheet.setVisibility(View.GONE);
		}
		...
		btnCancelAppointmentN.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				RequestParams params = new RequestParams();
				params.put("app_id", DATA.drAppointmentModel.getId());
				ApiManager apiManager = new ApiManager(ApiManager.PATIENT_CANCEL_APPTMNT,"post",params,apiCallBack, activity);
				apiManager.loadURL();
			}
		});

		btnRescheduleAppointment.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                openActivity.open(ActivityDoctorSlots.class, true);
            }
        });



		DATA.selectedLiveCare = new MyAppointmentsModel();//ActivityTcmDetails requirement
		DATA.selectedUserCallId = DATA.drAppointmentModel.getPatient_id();
		DATA.selectedUserCallName = DATA.drAppointmentModel.getFirst_name()+" "+DATA.drAppointmentModel.getLast_name();
		DATA.selectedUserCallImage = DATA.drAppointmentModel.getPatient_image();
		btnGoToPatientSheet.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//DATA.isVideoCallFromLiveCare = true;//for remove pop after call - "isFromAppointment" issue to be resolve then open this
				openActivity.open(ActivityTcmDetails.class, true);
			}
		});
	...
	public void fetchDataCallback(String httpStatus, String apiName, String content) {
		...
		}else if(apiName.equalsIgnoreCase(ApiManager.PATIENT_CANCEL_APPTMNT)){
			try {
				JSONObject jsonObject = new JSONObject(content);
			} catch (JSONException e) {
				e.printStackTrace();
