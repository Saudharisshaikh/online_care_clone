package com.app.mdlive_dr.reliance.reschedule;
...
public class ActivityDoctorSlots extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(getSupportActionBar() != null){
			getSupportActionBar().setTitle("Next Appointment");
		...
		tvProviderName.setText(prefs.getString("first_name", "")+ " "+prefs.getString("last_name", ""));
		tvProviderDesig.setText(prefs.getString("designation", ""));
		DATA.loadImageFromURL(prefs.getString("image", ""), R.drawable.icon_call_screen, ivProvider);
		...
		btnContinue.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if(TextUtils.isEmpty(slot_id)){
					...
					String msg = "Are you sure ? Do you want to book appointment with "+DATA.selectedUserCallName+" at "+tvSelectedDate.getText().toString();
	...
	public void bookAppointment() {
		...
		params.put("old_slot", DATA.drAppointmentModel.slot_id);
		params.put("slot_id", slot_id);
		params.put("appointment_date", etDateApt.getText().toString().trim());

		ApiManager apiManager = new ApiManager(ApiManager.UPDATE_APPOINTMENT,"post",params,apiCallBack, activity);
		apiManager.loadURL();
	...
	public void fetchDataCallback(String httpStatus, String apiName, String content) {
		...
		}else if(apiName.equalsIgnoreCase(ApiManager.UPDATE_APPOINTMENT)){
			//{"success":1,"message":"Appointment has been rescheduled."}
			try {
				JSONObject jsonObject = new JSONObject(content);
