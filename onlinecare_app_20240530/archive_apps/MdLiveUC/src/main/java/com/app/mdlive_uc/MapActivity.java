...
import com.app.mdlive_uc.adapter.LvLivecareDoctorsAdapter;
import com.app.mdlive_uc.adapter.NearbyHospitalsAdapter;
import com.app.mdlive_uc.api.ApiManager;
import com.app.mdlive_uc.model.HospitalsBean;
import com.app.mdlive_uc.model.NearbyDoctorBean;
import com.app.mdlive_uc.services.GPSTracker;
...
//	                R.id.map)).getMap();
	...
	public void getNearByDrs(String zipCode) {
		 
		AsyncHttpClient client = new AsyncHttpClient();
		ApiManager.addHeader(activity,client);
		DATA.showLoaderDefault(activity,"");

		String reqURL = DATA.baseUrl+ApiManager.PATIENT_COMPANIES_DOC+prefs.getString("id","");
		//String reqURL = DATA.baseUrl+"/searchDoctor/"+zipCode;
		...
		client.get(reqURL, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--responce in getNearByDrs: "+content);
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: searchDoctor, http status code: "+statusCode+" Byte responce: "+response);
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					...
					System.out.println("--responce in failure searchDoctor: "+content);
		...
		client.get(DATA.baseUrl+"/alldoctors/", new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--responce in alldoctors: "+content);
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: alldoctors, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("--responce in failure alldoctors: "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
	...
	public void getNearByHospitals(double lat,double lng) {
		...
		client.get(DATA.baseUrl+"/getHospitals/"+lat+"/"+lng, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					String content = new String(response);
					System.out.println("--responce in getNearByHospitals: "+content);
					try {
						JSONObject jsonObject = new JSONObject(content);
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: getNearByHospitals, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("--responce in failure getNearByHospitals: "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
