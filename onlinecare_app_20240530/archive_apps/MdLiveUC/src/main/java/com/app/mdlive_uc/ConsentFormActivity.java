		...
		client.post(DATA.baseUrl+"/genericConsent", params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--reaponce in saveFormData "+content);
		...
		client.post(DATA.baseUrl+"/getGenericConsent", params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--reaponce in getGenericConsent "+content);
					try {
...
//					etDOB.setText("");
					...
					e.printStackTrace();
					System.out.println("-- responce onsuccess: getGenericConsent, http status code: "+statusCode+" Byte responce: "+response);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
				}
			}
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("-- getGenericConsent on fail "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
