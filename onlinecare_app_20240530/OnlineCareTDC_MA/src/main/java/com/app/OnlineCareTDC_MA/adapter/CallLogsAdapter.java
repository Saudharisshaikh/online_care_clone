...
public class CallLogsAdapter extends ArrayAdapter<CallLogBean> {
	...
	public CallLogsAdapter(Activity activity,ArrayList<CallLogBean> callLogBeans) {
		super(activity, R.layout.lv_call_logs_row, callLogBeans);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		DATA.loadImageFromURL( callLogBeans.get(position).getImage(), R.drawable.icon_call_screen,viewHolder.ivCallLog);
		viewHolder.tvCallLogName.setText(callLogBeans.get(position).getFirst_name()+" "+callLogBeans.get(position).getLast_name());
		viewHolder.tvCallLogTime.setText(callLogBeans.get(position).getDateof());
		...
		client.get(reqURL, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] responseAPI) {
				...
				try{
					...
					e.printStackTrace();
					DATA.print("-- responce onsuccess: "+reqURL+", http status code: "+statusCode+" Byte responce: "+responseAPI);
