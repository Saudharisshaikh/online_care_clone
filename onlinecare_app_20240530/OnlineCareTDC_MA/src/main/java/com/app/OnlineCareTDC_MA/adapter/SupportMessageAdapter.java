...
//		filter(DATA.selectedDrId);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (supportMessageBeans.get(position).type.equalsIgnoreCase("coordinator")) {
			...
			//UrlImageViewHelper.setUrlDrawable(viewHolder.ivMsgMe, prefs.getString("image", ""), R.drawable.icon_call_screen);
			DATA.loadImageFromURL(prefs.getString("image", ""), R.drawable.icon_call_screen, viewHolder.ivMsgMe);
		}

		viewHolder.tvMsgTimeMe.setVisibility(View.GONE);
		viewHolder.tvMsgTimeOther.setVisibility(View.GONE);
