package com.app.Olc_MentalHealth.paypal;
...
import static com.app.Olc_MentalHealth.util.SharedPrefsHelper.PKG_AMOUNT;
...
import com.app.Olc_MentalHealth.b_health2.ActivityDoctorSlots;
import com.app.Olc_MentalHealth.b_health2.ActivityIcWaitingRoom;
import com.app.Olc_MentalHealth.b_health2.LvImmeCareProvidersAdapter;
...
public class PaymentLiveCare extends BaseActivity {
	...
	public static final int REQUEST_CODE_PAYMENT = 125;
...
//    private static final int REQUEST_CODE_PROFILE_SHARING = 3;
	...
	public RadioButton radioPayPal, radioCreditCard, radioHealthInsurance, radioFreeCare;
	TextView tvUrgentCarePayment,tvUrgentCarePaymentNext;
	...
	public static boolean isBraintreePaymentDone = false;
	@Override
	protected void onResume() {
		super.onResume();
		if(isBraintreePaymentDone){
			//DATA.livecarePaymentPaypalInfo = jsonObject.getString("id");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvUrgentCarePayment = findViewById(R.id.tvUrgentCarePayment);
		tvUrgentCarePaymentNext = findViewById(R.id.tvUrgentCarePaymentNext);
		radioCreditCard = findViewById(R.id.radioCreditCard);
		radioHealthInsurance = findViewById(R.id.radioHealthInsurance);
		radioFreeCare = findViewById(R.id.radioFreeCare);
			...
			public void onClick(View view) {
				switch (view.getId()) {
					...
					case R.id.radioCreditCard:
						...
						//Ahmer add credit card button here 21-06-2022 - Sir Jamal
						/*payment_typeIC = "";
						insuranceOrCredit = false;
						DATA.isFreeCare = false;//for launch paypal in onBuyPressed
						rgFreeOptions.setVisibility(View.GONE);
						...
						//Ahmer comment this code 21-06-2022
						DATA.isFreeCare = true;
						insuranceOrCredit = true;
	...
	public void onBuyPressed(View v) {
		...
		if(!checkInternetConnection.isConnectedToInternet()){
			...
			else if (payment_typeIC.equals("NewInsurance"))
			{
				return;
			}

			if (DATA.isFreeCare) {
				if (LvImmeCareProvidersAdapter.careStatus){
					LvImmeCareProvidersAdapter.careStatus = false;
					//ActivityDoctorSlots.doctorBean = doctorBeans.get(position);
					//AHMER'S CODE FOR MH APP
	...
	public void fetchDataCallback(String httpStatus, String apiName, String content) {
		...
		if(apiName.equalsIgnoreCase(ApiManager.CHECK_FREE_CODE)){
			...
			try {
				...
				//stripe_pub_key = "pk_test_YGQVbRrz1utJrlEGgXcHp95f00ipD4Kred"; //= stripe.getString("stripe_pub_key");
				paymentMessage = jsonObject.getString("message");

				JSONObject braintree = data.getJSONObject("braintree");
				String brain_auth_key = braintree.getString("brain_auth_key");
				sharedPrefsHelper.save("brain_auth_key", brain_auth_key);
...
*/
	...
	public void stripeCharge(String payment_token){
		RequestParams params = new RequestParams();
		params.put("stripe_token", payment_token);
	...
	public void getlabel()
	{
		...
		if(! TextUtils.isEmpty(prefsDataStr)){
			try {
				...
				String labelUrgentCarePayment = jsonObject.getString("payment_charge_label");
				String labelUrgentCarePaymentNext = jsonObject.getString("next_payment_charge_label");
				tvUrgentCarePaymentNext.setText(labelUrgentCarePaymentNext);
				tvUrgentCarePayment.setText(labelUrgentCarePayment +" "+"$ "+pkgAmount);
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}
