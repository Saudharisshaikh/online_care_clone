...
import com.app.omranpatient.model.SpecialityModel;
...
public class Database {
	...
	String DB_PATH = "data/data/com.app.omranpatient/databases/";
	...
	public ArrayList<SymptomsModel> getAllSymptoms() {
		...
		//String query1 = "select * from symptoms where symptom_id > 893 order by symptom_name asc";
	...
	public ArrayList<ConditionsModel> getAllConditions(String symptomId) {
		...
		//String query1 = "select * from conditions where symptom_id='"+symptomId+"' OR condition_name = 'Other'";
		//String query1 = "select * from conditions where symptom_id='"+symptomId+"' order by condition_name asc";

		String query1 = "select * from conditions where symptom_id='"+symptomId+"' order by sortby asc";
		System.out.println("--online care db get conditions query: "+query1);
		Cursor cursor = sqLiteDatabase.rawQuery(query1, null);		
		...
		{		
			...
			do {
				...
				System.out.println("--online care db get conditions: "+cursor.getString(cursor.getColumnIndex("condition_name")));
	...
	public void insertFirstLoginUser(String user_id)
	{
		open();

		String query = "insert into first_login_check (user_id) values('"+user_id+"')";
		sqLiteDatabase.execSQL(query);
		close();
	}
	...
	public void emptyFirstLoginUser()
	{
		...
		String query = "delete from first_login_check";
		sqLiteDatabase.execSQL(query);
		close();
	}


	public boolean alreadyLoggedIn(String user_id)
	{
		open();

			final String MY_QUERY = "SELECT * FROM first_login_check where user_id='"+user_id+"'";
			Cursor cur = sqLiteDatabase.rawQuery(MY_QUERY, null);
			if(cur.moveToFirst()) {
	...
	public ArrayList<String> getAllNotif() {
		...
		System.out.println("--query in getAllFiles: "+query1);
		Cursor cursor = sqLiteDatabase.rawQuery(query1, null);
