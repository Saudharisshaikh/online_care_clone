...
import static com.app.mhcsn_spe.util.DATA.baseUrl;
...
//	public static final String uploadFilesURL = "http://13.95.108.74/dynamicData/api/services/app/FormApi/uploadFiles";
	...
	public ApiManagerJSON(String api, String getOrPost, JSONObject jsonParams, ApiCallBack apiCallBack, Activity activity) {
		...
		System.out.println("-- url in ApiManager constructor: "+customURL);

		if(jsonParams != null){
			System.out.println("-- params in : "+api+" : "+jsonParams.toString());
		}
	}
	...
	public void loadURL() {
		...
		//if(sharedPrefsHelper.get("isLogin",false)){
			...
			//System.out.println("-- authToken : "+"Bearer "+sharedPrefsHelper.getUser().accessToken);
		//}else {
				 ...
				 client.post(activity,customURL,null,entity,"application/json", new AsyncHttpResponseHandler() {
			        //client.post(BASE_URL+api,params, new AsyncHttpResponseHandler() {
                     ...
                     public void onSuccess(int statusCode, Header[] headers, byte[] response) {
						 ...
						 try{
							 if(response != null){
								 ...
								 System.out.println("-- in activity "+apiCallBack.getClass().getName().replace(activity.getPackageName(),"")+": status: "+
								 ...
								 System.out.println("-- responce onsuccess: "+api+" :Error status code: "+statusCode+" Byte responce: "+response);
							 }
						 }catch (Exception e){
							 e.printStackTrace();
						 }
					 ...
					 public void onProgress(long bytesWritten, long totalSize) {
						 ...
						 //System.out.println("-- Progress: "+((bytesWritten/totalSize)*100));
	...
	/*public static void addHeaders(AsyncHttpClient client){
		...
		System.out.println("-- authToken : "+"Bearer "+SharedPrefsHelper.getInstance().getUser().accessToken);
	}*/
}
