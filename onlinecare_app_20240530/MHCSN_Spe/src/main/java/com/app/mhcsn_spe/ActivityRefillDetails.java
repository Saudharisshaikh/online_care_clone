...
import com.app.mhcsn_spe.adapter.LvIdentifyPatientAdapter;
import com.app.mhcsn_spe.api.ApiManager;
import com.app.mhcsn_spe.model.DrugBean;
import com.app.mhcsn_spe.model.DrugRefillBean;
import com.app.mhcsn_spe.model.IdentifyPatientBean;
import com.app.mhcsn_spe.model.PotencyUnitBean;
...
import com.app.mhcsn_spe.util.DatePickerFragment;
		...
		client.post(reqURL , params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--reaponce in getMyPatientsByDate: "+content);
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					...
					System.out.println("-- onfailure getMyPatientsByDate : "+content);
		...
		client.post(reqURL, params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--reaponce in linkRefillWithPatient "+content);
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("-- onfailure linkRefillWithPatient : "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
		...
		client.get(reqURL , new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] response) {
				...
				try{
					...
					System.out.println("--reaponce in denieUnknownRefill "+content);
	...
	public void refillSummary(String prescription_id,String refill_id) {
		...
		client.post(reqURL, params, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					String content = new String(errorResponse);
					System.out.println("-- onfailure refillSummary : "+content);
					new GloabalMethods(activity).checkLogin(content);
					customToast.showToast(DATA.CMN_ERR_MSG,0,0);
	...
	ArrayList<DrugBean> drugBeans;
		...
		DATA.showLoaderDefault(activity, "");
		 ...
		 client.post(reqURL, params, new AsyncHttpResponseHandler() {
			 ...
			 public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				 ...
				 try {
					 String content = new String(errorResponse);
					 System.out.println("-- onfailure getDrugs : "+content);
					 new GloabalMethods(activity).checkLogin(content);
					 customToast.showToast(DATA.CMN_ERR_MSG,0,0);
		    ...
		    spinnerDrugName.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> arg0, View arg1,
					...
					try {
						...
						System.out.println("-- excepton in setting refill spinner position");
						e.printStackTrace();
					}
	 ...
	 EditText editText;
		...
		public void sendRefillResponse(String status,String drug_id,String start_date,String end_date,String refill_id,
			...
			System.out.println("-- params in apiName : "+apiName+" : "+params.toString());
