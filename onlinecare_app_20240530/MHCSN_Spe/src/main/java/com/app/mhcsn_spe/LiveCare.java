...
import com.app.mhcsn_spe.adapter.LiveCareAdapter;
...
import com.app.mhcsn_spe.model.ReportsModel;
import com.app.mhcsn_spe.util.DATA;
import com.app.mhcsn_spe.util.Database;
...
public class LiveCare extends BaseActivity implements ApiCallBack,GloabalSocket.SocketEmitterCallBack{
	...
	public static int liveOrReferred = 1;//0=livecheckup, 1=refferedCheckup
	...
	private void getLiveCarePatiensList() {
		String apiname = liveOrReferred == 1 ? ApiManager.GET_REFFERED_PATIENTS : ApiManager.GET_MY_LIVE_CHECKUPS;
...
//					msg = jsonObject.getString("msg");
		...
		}else if(apiName.equals(ApiManager.GET_REFFERED_PATIENTS)){
			try {
				jsonObject = new JSONObject(content);
				status = jsonObject.getString("status");
//					msg = jsonObject.getString("msg");
				...
				if(status.equals("success")){
					...
					String appts = jsonObject.getString("referredcheckups");
					JSONArray appointmentsArray = new JSONArray(appts);
					...
					for(int i = 0; i<appointmentsArray.length(); i++) {
						//			if(appointmentsArray.getJSONObject(i).getString("only_reports").equals("0")){
						temp = new MyAppointmentsModel();
						temp.id = appointmentsArray.getJSONObject(i).getString("patient_id");
						temp.liveCheckupId = appointmentsArray.getJSONObject(i).getString("ref_checkup_id");
						temp.first_name = appointmentsArray.getJSONObject(i).getString("first_name");
						temp.last_name = appointmentsArray.getJSONObject(i).getString("last_name");
						temp.symptom_name = appointmentsArray.getJSONObject(i).getString("symptom_name");
						temp.condition_name = appointmentsArray.getJSONObject(i).getString("condition_name");
						temp.description = appointmentsArray.getJSONObject(i).getString("description");
						temp.patients_qbid = appointmentsArray.getJSONObject(i).getString("old_checkup_id");//patients_qbid   for remove from queue, this was also used by maaz in sendPresscription but now not used in sendPresscription
						temp.datetime = appointmentsArray.getJSONObject(i).getString("datetime");
						...
						temp.refered_from = appointmentsArray.getJSONObject(i).getString("refered_from");
...
//				            "patient_id": "17",
//				            "report_id": "28",
//				            "report_name": "941620306ultrasound.jpg",
//				            "file_display_name": "ultrasound"

							temp1.name = reportsArray.getJSONObject(j).getString("file_display_name");
							//temp1.url = DATA.imageBaseUrl+reportsArray.getJSONObject(j).getString("report_name");
                            temp1.url = reportsArray.getJSONObject(j).getString("report_name");
							temp1.patientID = reportsArray.getJSONObject(j).getString("patient_id");//patient_id
					...
					customSnakeBar.showToast("Something went wrong, please try again");
				}
