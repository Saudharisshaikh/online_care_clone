package com.app.mhcsn_spe.reliance.assessment.new_assesment;
...
import static com.app.mhcsn_spe.PatientMedicalHistory.preventScrollViewFromScrollingToEdiText;
...
public class ActivityOCD_Form extends BaseActivity implements AssessSubmit{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(getSupportActionBar() != null){
			...
			if(formFlag == 1){
				getSupportActionBar().setTitle(OCD_FORM_NAME);
				tvFormDesc.setText(OCD_FORM_NAME);
			}else if(formFlag == 2){
				getSupportActionBar().setTitle(FCSAS_FORM_NAME);
				tvFormDesc.setText(FCSAS_FORM_NAME);
			}else if(formFlag == 3){
				getSupportActionBar().setTitle(PANIC_ATACK_FORM_NAME);
				tvFormDesc.setText(PANIC_ATACK_FORM_NAME);
			}else if(formFlag == 4){
				getSupportActionBar().setTitle(SCOFF_FORM_NAME);
				tvFormDesc.setText(SCOFF_FORM_NAME);
			}
		}
			...
			for (int i = 0; i < questions.length(); i++) {
				...
				if(isEdit){
					...
					if (json instanceof JSONObject){
						System.out.println("-- Form Data is of type JSONObject");
						...
						System.out.println("-- Form Data is of type JSONArray");
						//view data part starts
						try {
    ...
    public void submitForm2(String is_lock){
        ...
        try {
            ...
            for (int i = 0; i < ocdFormBeans.size(); i++) {
                System.out.println("ans["+i+"]"   + " = "  + ocdFormBeans.get(i).score);
