...
public class HistoryMediAdapter extends ArrayAdapter<String> {
	...
	public HistoryMediAdapter(Activity activity, ArrayList<String> medications) {
		...
		try {
			((PatientMedicalHistoryNew) activity).tvViewMed.setText("View ("+medications.size()+")");
		}catch (Exception e){
			e.printStackTrace();
		}
	}
	...
	public void editMedDialog(final int listPos){
		...
		//note: this is search btn on edit. hidden currently due to refresh issue on show all medi dialog GM
		ivSearchM.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				String med = etEditMed.getText().toString();
				if(med.isEmpty()){
				...
				if(activity instanceof PatientMedicalHistoryNew){
					try {
						((PatientMedicalHistoryNew)activity).showDrugDialog(med.split("\\s+")[0],true,listPos);
						medicationsDialog.dismiss();
					}catch (Exception e){
						e.printStackTrace();

						((PatientMedicalHistoryNew)activity).showDrugDialog(med,true,listPos);
						medicationsDialog.dismiss();
					}
				}
			}
