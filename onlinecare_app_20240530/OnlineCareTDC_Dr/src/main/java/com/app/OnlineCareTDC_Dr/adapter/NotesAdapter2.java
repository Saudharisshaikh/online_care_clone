...
import com.app.OnlineCareTDC_Dr.ActivityCallLogs;
import com.app.OnlineCareTDC_Dr.ActivityEncounterNotes;
import com.app.OnlineCareTDC_Dr.ActivitySoapNotesEditNew;
import com.app.OnlineCareTDC_Dr.ActivityTelemedicineServices;
import com.app.OnlineCareTDC_Dr.AfterCallDialogEmcura;
...
public class NotesAdapter2 extends ArrayAdapter<NotesBean> {
    ...
    String skinValue;
    String otherValue;
    String neurologicValue;
    String extremitiesValue;
    String headValue;
    String lungsValue;
    ...
    static class ViewHolder {
        ...
        TextView tvBillingHistory, tvBillingServices, tvBillingObjective, tvBillingFamily,
                ...
                tvHomeCareReferal,
                tvOTNotesTemp, tvOTNotesHeight, tvOTNotesWeight, tvOTNotesBMI, tvSympExp,tvNotesSymptoms;//tvBillingPatientName
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            viewHolder.tvNotesSymptoms = (TextView) convertView.findViewById(R.id.tvNotesSymptoms);
            ...
            viewHolder.btnforwardnote = convertView.findViewById(R.id.btnforwardnote);
            ...
            convertView.setTag(R.id.tvNotesSymptoms, viewHolder.tvNotesSymptoms);
            ...
            convertView.setTag(R.id.btnforwardnote, viewHolder.btnforwardnote);
        } else {
        ...
        viewHolder.tvNotesSymptoms.setText(notesBeans.get(position).symptom);
        ...
        if (!notesBeans.get(position).examination.isEmpty()) {
            try {
                JSONObject examinationJson = new JSONObject(notesBeans.get(position).examination);

                headValue = examinationJson.getString("head");
                eyesValue = examinationJson.getString("eyes");
                earValue = examinationJson.getString("ear");
                noseValue = examinationJson.getString("nose");
                throatValue = examinationJson.getString("throat");
                heartValue = examinationJson.getString("heart");
                lungsValue = examinationJson.getString("lungs");
                extremitiesValue = examinationJson.getString("extremities");
                neurologicValue = examinationJson.getString("neurologic");
                chestValue = examinationJson.getString("chest");
                skinValue = examinationJson.getString("skin");
                giValue = examinationJson.getString("gi");
                guValue = examinationJson.getString("gu");
                otherValue = examinationJson.getString("other");


                /*Iterator<String> iter = examinationJson.keys();
                while (iter.hasNext()) {
                    String key = iter.next();
                    try {
                        Object value = examinationJson.get(key);
        ...
        viewHolder.tvExamination.setText("Head: " + headValue + "\n" +"Eyes: " + eyesValue + "\n" +"Ear: " + earValue + "\n"
                +"Nose: " + noseValue + "\n" +"Throat: " + throatValue + "\n" +"Heart: " + heartValue + "\n"
                +"Lungs: " + lungsValue + "\n" +"Chest: " + chestValue + "\n" +"Extremities: " + extremitiesValue + "\n" +"Neurologic: " + neurologicValue + "\n"
                +"Skin: " + skinValue + "\n" +"GI: " + giValue + "\n" +"GU: " + guValue + "\n"  +"Other: " + otherValue + "\n");
        ...
        if (activity instanceof ActivityCallLogs || activity instanceof AfterCallDialogEmcura || activity instanceof ActivityTelemedicineServices || activity instanceof ActivityEncounterNotes) {
        ...
        viewHolder.btnforwardnote.setOnClickListener(view ->
        {
            if (activity instanceof ActivityCallLogs) {
                ((ActivityCallLogs) activity).forwardNote(notesBeans.get(position).id);
            } else if (activity instanceof ActivityEncounterNotes) {
                ((ActivityEncounterNotes) activity).forwardNote(notesBeans.get(position).id);
            }
        });
