...
import com.app.OnlineCareTDC_Dr.adapter.CategoriesAdapter;
...
import com.app.OnlineCareTDC_Dr.permission.PermissionsActivity;
import com.app.OnlineCareTDC_Dr.permission.PermissionsChecker;
import com.app.OnlineCareTDC_Dr.reliance.therapist.ActivityReferedPatients;
import com.app.OnlineCareTDC_Dr.services.GPSTracker;
import com.app.OnlineCareTDC_Dr.util.ActionEditText;
import com.app.OnlineCareTDC_Dr.util.CallWebService;
...
import com.app.OnlineCareTDC_Dr.util.DatePickerFragmentBtn;
...
import com.app.OnlineCareTDC_Dr.util.SharedPrefsHelper;
...
public class MainActivityNew extends AppCompatActivity implements ApiCallBack, GloabalSocket.SocketEmitterCallBack{
	...
	CardView tvLiveCare;//,tvCovidList;
	...
	TextView tvMessagesBadge,tvLiveCareBadge,tvAppointmentBadge,tvPrescriptionsBadge,tvRefillsBadge,
			tvGroupsBadge,tvAddDoctorBadge,tvSoapRefBadge,tvCovidBadge,tvdrSchedulelabel;
	CheckBox cbAvailableForeLiveCare;
	String is_available_onlinecare = "0";
	Button btnOffline;
	//boolean isOnline = true;

	LinearLayout layPermissions,layDoctorSchedule;
	Button btnReviewPerm,btnExitApp,btnReviewChkDrSchdle;
	...
	protected void onResume() {
		...
		String datefromserver = prefs.getString("schedule_end_date" , "");
		DATA.print("-- datefromserver " + datefromserver);
		//String dates = new SimpleDateFormat("yyyy/MM/dd", Locale.getDefault()).format(new Date());
		//DATA.print("-- current dates " + dates);

		/*SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date strDate = null;
		try {
			strDate = sdf.parse(datefromserver);
		...
		SimpleDateFormat sdf = new SimpleDateFormat(   "MM/dd/yyyy", Locale.ENGLISH);//"yyyy-MM-dd"
		Date dateFromServer;
		Date currentDate;

		try {
			dateFromServer = sdf.parse(datefromserver); //sdf.parse(datefromserver);  // Replace with your datefromserver value

			String todayDateStr = sdf.format(new Date());
			currentDate = sdf.parse(todayDateStr);  // Use the current date

			int compare = currentDate.compareTo(dateFromServer);

			DATA.print("-- before(dateFromServer " + compare);

			if (compare > 0) {
				layDoctorSchedule.setVisibility(View.VISIBLE);

			}
			else {
				layDoctorSchedule.setVisibility(View.GONE);
				DATA.print("-- after date from server");
			}
		} catch (ParseException e) {
			e.printStackTrace();
		}

		/*if (dates.compareTo(datefromserver) > 0) {
				layDoctorSchedule.setVisibility(View.VISIBLE);
			} else if (dates.compareTo(datefromserver) < 0) {
				layDoctorSchedule.setVisibility(View.GONE);
			}
			else {
				layDoctorSchedule.setVisibility(View.GONE);
		...
		/*try {
			String date = prefs.getString("schedule_end_date" , "");;
...
//		exportDB();
		...
		tvdrSchedulelabel = findViewById(R.id.tvdrSchedulelabel);
...
//		};
		...
		DATA.allCategories.add(new CategoriesModel("1", "Waiting Room",R.drawable.ic_drawer_elivecare_n));
		DATA.allCategories.add(new CategoriesModel("4", "Doc To Doc",R.drawable.ic_drawer_doc_todoc_n));
		DATA.allCategories.add(new CategoriesModel("2", "My Patients",R.drawable.ic_drawer_mypatients));
		DATA.allCategories.add(new CategoriesModel("3", "Lab Request",R.drawable.ic_drawer_lab_request));
		DATA.allCategories.add(new CategoriesModel("12", "My Messages",R.drawable.ic_drawer_my_messages));
		DATA.allCategories.add(new CategoriesModel("13", "Call History",R.drawable.ic_drawer_call_history));
...
//		DATA.allCategories.add(new CategoriesModel("5", "My Appointments",R.drawable.ic_drawer_my_appointment));
		...
		tvdrSchedulelabel.setText(prefs.getString("schedule_expire_msg" , ""));
		...
		layDoctorSchedule = findViewById(R.id.layDoctorSchedule);
		btnReviewChkDrSchdle = findViewById(R.id.btnReviewChkDrSchdle);

		btnReviewChkDrSchdle.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				openActivity.open(DoctorScheduleNew.class, false);
			}
...
//				setTitle(patientArry[position]);								
	...
	final String SUPPORT_MESSAGE = "We are currently available from 9am to 5pm live on our website at www.onlinecare.com - You may also email us at: support@totaldirectcare.com";
	final String SUPPORT_EMAIL = "support@totaldirectcare.com";
	...
	public void getLabels(){
		String app_general_labels = prefs.getString("app_general_labels", "");
	...
	private void parseLabelsData(String content){
		try {
			...
			JSONArray jsonArray = objectHeentValue.getJSONArray("ear");

			if(prefs.getBoolean("debug_logs", false)){
				DATA.print("-- heentValuesEar " + objectHeentValue);
			}
	...
	public void setDrScheDate(String date)
	{
		tvdrSchedulelabel.setText(date);
	}
