...
public class DrugsAdapter extends ArrayAdapter<DrugBean> {
	...
	static class ViewHolder {
		...
		TextView tvDrugName,tvDrugStrength,tvDrugDosageForm,tvDrugQuantity,tvDrugDuration,tvDrugPotencyCode,tvDrugInstructions,tvDrugRefils,tvDrugNum,tvRxFillIndec,tvInstrucationNote;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			...
			viewHolder.tvRxFillIndec = (TextView) convertView.findViewById(R.id.tvRxFillIndec);
			viewHolder.tvInstrucationNote = (TextView) convertView.findViewById(R.id.tvInstrucationNote);
			...
			convertView.setTag(R.id.tvRxFillIndec , viewHolder.tvRxFillIndec);
		...
		//viewHolder.tvRxFillIndec.setText(DATA.drugBeans.get(position).rxfillIndicator);
		String rxIndecator = DATA.drugBeans.get(position).rxfillIndicator;
		String instructionNote = DATA.drugBeans.get(position).instructionNote;
		if (rxIndecator == null) {
			viewHolder.tvRxFillIndec.setText("");
		}
		else {
			viewHolder.tvRxFillIndec.setText(DATA.drugBeans.get(position).rxfillIndicator);
		}
		if (instructionNote == null) {
			viewHolder.tvInstrucationNote.setText("");
		}
		else {
			viewHolder.tvInstrucationNote.setText(DATA.drugBeans.get(position).instructionNote);
		}
