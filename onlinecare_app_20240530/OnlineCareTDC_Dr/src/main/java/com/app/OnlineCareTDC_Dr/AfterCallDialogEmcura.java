...
import com.app.OnlineCareTDC_Dr.adapter.LabReportsAdapter;
import com.app.OnlineCareTDC_Dr.adapter.TelemedicineAdapter;
import com.app.OnlineCareTDC_Dr.adapter.TelemedicineAdapter2;
import com.app.OnlineCareTDC_Dr.api.ApiManager;
import com.app.OnlineCareTDC_Dr.model.IcdCodesData;
import com.app.OnlineCareTDC_Dr.model.IcdcodesCategories;
import com.app.OnlineCareTDC_Dr.model.LabReportBean;
import com.app.OnlineCareTDC_Dr.model.TelemedicineCatData;
import com.app.OnlineCareTDC_Dr.model.TelemedicineCategories;
import com.app.OnlineCareTDC_Dr.model.TimeDiff;
import com.app.OnlineCareTDC_Dr.reliance.DialogAllEncNotes;
...
public class AfterCallDialogEmcura extends BaseActivity {
	...
	Button btnAfterCallEndConv,btnAfterCallAddNotes,btnAfterCallAddNotePresc,btnAfterCallRefillPresc, btnAfterCallLabReq,btnAfterCallGetICDCodes,btnAfterCallNotNow;
	LinearLayout layEndConvOptions;

	boolean isForRemoveFromQueueFromUcDet  = false;
	public static boolean autoNoteStatus = false; //status for remove autoNote dialog only
...
//		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String encounter_notesValue = prefs.getString("encounter_notesDurringCall", "");
		if (encounter_notesValue.isEmpty())
		{
			...
			editor.apply();
			System.out.println("-- sharedprefrence " + sharedPreferences.getString("encounter_notesDurringCall", ""));
		}
		else
		{
			SharedPreferences sharedPreferences = getSharedPreferences("my_preferences", Context.MODE_PRIVATE);
		...
		tvDialogAddSOAP.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				//Ahmer work after call with sir jamal 07-03-2023
				ActivitySOAP.addSoapFlag = 1;
				activity.startActivity(new Intent(activity, ActivitySoapNotesNew.class));
		...
		btnAfterCallGetICDCodes = findViewById(R.id.btnAfterCallGetICDCodes);
		btnAfterCallNotNow = findViewById(R.id.btnAfterCallNotNow);

		//new work after sir jamal call 07/03/2023
			...
			public void onClick(View v) {
				switch (v.getId()){
					...
					case R.id.btnAfterCallLabReq:
						getLabReports();
						break;
					case R.id.btnAfterCallGetICDCodes:
						showICDCodesDialog();
						break;
		...
		btnAfterCallGetICDCodes.setOnClickListener(onClickListener);
...
//		params.put("appointment_id", DATA.selectedUserAppntID);
	...
	public void fetchDataCallback(String httpStatus, String apiName, String content) {
		...
		else if(apiName.equalsIgnoreCase(ApiManager.GET_ICD_CODES_SERVICES)){

			parseIcdCodesData(content);
		}
		else if(apiName.equalsIgnoreCase(ApiManager.SAVE_LIVECARE_ICDCODES)) {
			try {
				JSONObject jsonObject = new JSONObject(content);
				String status = jsonObject.getString("status");
				String msg = jsonObject.getString("message");
				if (status.equalsIgnoreCase("success")) {
					dialogIcdCodesSer.dismiss();
	...
	//Ahmer Work
	public void parseIcdCodesData(String content){
		icdcodesCategories = new ArrayList<>();
		IcdcodesCategories temp;
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				JSONObject obj = jsonArray.getJSONObject(i).getJSONObject("cateogries");

				String id = obj.getString("id");
				String category_name = obj.getString("category_name");

				JSONArray codesArray = obj.getJSONArray("codes");
				ArrayList<IcdCodesData> icdData = new ArrayList<>();
				IcdCodesData icdCodesData;
				for (int j = 0; j < codesArray.length(); j++) {
					String idCode = codesArray.getJSONObject(j).getString("id");
					String category_idICD = codesArray.getJSONObject(j).getString("category_id");
					String code = codesArray.getJSONObject(j).getString("code");
					String desc = codesArray.getJSONObject(j).getString("desc");

					icdCodesData = new IcdCodesData(idCode, category_idICD, code, desc, false);
					icdData.add(icdCodesData);
					//icdcodesCategories = null;
				}

				temp = new IcdcodesCategories(id, category_name , icdData);
				icdcodesCategories.add(temp);

			}
			IcdCodesDataAdapter adapter = new IcdCodesDataAdapter(activity, icdcodesCategories);
			WrapperExpandableListAdapter wrapperAdapter = new WrapperExpandableListAdapter(adapter);
			if(lv_icdCodesData != null){
				lv_icdCodesData.setAdapter(wrapperAdapter);
			}
	...
	ArrayList<IcdcodesCategories> icdcodesCategories;
	private void getICDCodesServices() {

		RequestParams params = new RequestParams();
		ApiManager apiManager = new ApiManager(ApiManager.GET_ICD_CODES_SERVICES,"post",params,apiCallBack, activity);
		apiManager.loadURL();
	}

	//Ahmer add icdCodes here - 25/05/2022
	FloatingGroupExpandableListView lv_icdCodesData;
	Dialog dialogIcdCodesSer;
	Button btnSaveICDCODES;
	public void showICDCodesDialog(){
		dialogIcdCodesSer = new Dialog(activity,R.style.TransparentThemeH4B);
		dialogIcdCodesSer.requestWindowFeature(Window.FEATURE_NO_TITLE);
		//dialogStudioInfo.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
		//dialogAddTemplPresc.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
		dialogIcdCodesSer.setContentView(R.layout.dialog_icdcode_services);

		lv_icdCodesData = (FloatingGroupExpandableListView) dialogIcdCodesSer.findViewById(R.id.lv_icdCodesData);
		btnSaveICDCODES = (Button) dialogIcdCodesSer.findViewById(R.id.btnSaveICDCODES);

		OnClickListener onClickListener = new OnClickListener() {
			@Override
			public void onClick(View view) {
				// TODO Auto-generated method stub
				if (view.getId() == R.id.btnSaveICDCODES) {
					icdCodesProceed();
				}
			}
		};
		btnSaveICDCODES.setOnClickListener(onClickListener);
		ImageView ivClose = dialogIcdCodesSer.findViewById(R.id.ivClose);
		ivClose.setOnClickListener(v -> dialogIcdCodesSer.dismiss());



		WindowManager.LayoutParams lp = new WindowManager.LayoutParams();
		lp.copyFrom(dialogIcdCodesSer.getWindow().getAttributes());
		lp.width = WindowManager.LayoutParams.MATCH_PARENT;
		lp.height = WindowManager.LayoutParams.MATCH_PARENT;
		...
		dialogIcdCodesSer.setCanceledOnTouchOutside(false);
		dialogIcdCodesSer.show();
		dialogIcdCodesSer.getWindow().setAttributes(lp);
	...
	//Amer work =========== 27-05-2022
	StringBuilder sbSelectedIcdCodes;
	String icdCodes;
	public void icdCodesProceed() {

		if (icdcodesCategories != null) {
			sbSelectedIcdCodes = new StringBuilder();
			for (int i = 0; i < icdcodesCategories.size(); i++) {
				for (int j = 0; j < icdcodesCategories.get(i).icdCodesData.size(); j++) {

					if (icdcodesCategories.get(i).icdCodesData.get(j).isSelected) {
						sbSelectedIcdCodes.append(icdcodesCategories.get(i).icdCodesData.get(j).code);
						sbSelectedIcdCodes.append(",");
					}
				}
			}

			icdCodes = sbSelectedIcdCodes.toString();
			DATA.print("-- selected icdCodes: "+icdCodes);

//				if(dialogIcdCodesSer != null){
//					dialogIcdCodesSer.dismiss();
//				}
				if (icdCodes.isEmpty()) {
					customToast.showToast("Please select icd codes",0,0);}
				else { saveICDCodes(icdCodes); }

		} else {
			DATA.print("-- icdcodesCategories list is null !");
		}
	...
	//==========Ahmer work Save ICD Codes
	public void saveICDCodes(String icdCodes){
		RequestParams params = new RequestParams();
		params.put("patient_id",DATA.selectedUserCallId);
		params.put("code", icdCodes);
		...
		params.put("call_id", callId);
		//GM added call_id
		ApiManager apiManager = new ApiManager(ApiManager.SAVE_LIVECARE_ICDCODES,"post",params,apiCallBack, activity);
		apiManager.loadURL();
	}
	...
	public void initNoteDialog() {
		...
		ic_mike_LiveExtraInfo.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				startVoiceRecognitionActivity(etNote);
			}
		});
	...
	String note_textFinal = "";
	Dialog dialogNote;
	public void initNoteDialogFinal() {

		prefs = getSharedPreferences(DATA.SHARED_PREFS_NAME, Context.MODE_PRIVATE);
		dialogNote = new Dialog(activity);
		dialogNote.requestWindowFeature(Window.FEATURE_NO_TITLE);
		dialogNote.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
		dialogNote.getWindow().setGravity(Gravity.CENTER);
		//dialogNote.getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
		dialogNote.setContentView(R.layout.dialog_billwithnote_final);
		dialogNote.setCanceledOnTouchOutside(false);
		...
		ImageView ic_mike_LiveExtraInfo = dialogNote.findViewById(R.id.ic_mike_LiveExtraInfo);
		TextView tvSubmitNote = dialogNote.findViewById(R.id.tvSubmitNote);
		TextView tvNotNow = dialogNote.findViewById(R.id.tvNotNow);
		...
		String existingValue = prefs.getString("encounter_notesDurringCall", "");
		System.out.println("-- existingValue final " + existingValue);
		etNote.setText(""+existingValue);

		tvSubmitNote.setOnClickListener(arg0 -> {
			note_text = etNote.getText().toString().trim();
			if (note_text.isEmpty()) {
				customToast.showToast("Please enter your virtual visit note",0,0);
				etNote.setError("Please enter your virtual visit note");
...
((AfterCallDialogEmcura)activity).billWithoutNote("" , note_text , prefs.getString("id", ""));
}*/
									billWithoutNote("" , note_text);
								}
							})
							.setNegativeButton("Edit", null)
							.create()
