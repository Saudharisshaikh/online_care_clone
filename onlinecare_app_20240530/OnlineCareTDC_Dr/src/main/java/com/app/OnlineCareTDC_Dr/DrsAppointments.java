...
import com.app.OnlineCareTDC_Dr.adapter.DrAppointmentsAdapter;
...
import com.app.OnlineCareTDC_Dr.util.SpinnerCustom;
...
//		/prefs.getString("id", "")
        ...
        spDates.setOnItemSelectedListener(new SpinnerCustom.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View arg1, int pos, long arg3,boolean isUserSelect) {
                ...
                if(isUserSelect){
                    if (connection.isConnectedToInternet()) {
                        if (apptmntDates.get(pos).equals("All")) {
                            getDrsAppointments(prefs.getString("id", ""),"");
                        } else {
                    ...
                    System.out.println("-- getDrsAppointments if");
                }
               /* else if(isUserSelect && pos == 0){
                    if (connection.isConnectedToInternet()) {
                        getDrsAppointments(prefs.getString("id", "") , "");
                    } else {
                        customToast.showToast(DATA.NO_NETWORK_MESSAGE,0,0);
                    }
                    System.out.println("-- getDrsAppointmentsall else ");
            } */

                System.out.println("-- isUserSelect " + isUserSelect);
            }
    ...
    private void getDrsAppointments(String drId,final String apptmntDate) {
        ...
        String reqURL = DATA.baseUrl+"doctor/getDoctorAppointment";
