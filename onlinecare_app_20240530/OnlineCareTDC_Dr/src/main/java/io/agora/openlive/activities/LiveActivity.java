...
import com.app.OnlineCareTDC_Dr.ActivityInstatntConnect;
import com.app.OnlineCareTDC_Dr.AfterCallDialogEmcura;
import com.app.OnlineCareTDC_Dr.R;
import com.app.OnlineCareTDC_Dr.VCallModule;
import com.app.OnlineCareTDC_Dr.adapter.LvDoctorsAdapter;
import com.app.OnlineCareTDC_Dr.adapter.LvSpecialistAdapter;
import com.app.OnlineCareTDC_Dr.adapter.NurseAdapter;
...
import com.app.OnlineCareTDC_Dr.util.ReportsDialog;
...
public class LiveActivity extends RtcBaseActivity implements ApiCallBack {
    ...
    TextView tvVCallViewReports,tvVCallViewPtDetails, tvVCallViewAddNotes;
    ...
    //Ahmer work ========== 27-05-2022
    LvSpecialistAdapter lvSpecialistAdapter;
    public void getAllSpecialist() {
        AsyncHttpClient client = new AsyncHttpClient();
        DATA.showLoaderDefault(activity,"");
        ApiManager.addHeader(activity,client);
        String reqURL = DATA.baseUrl+"getSpecialties";

        DATA.print("-- Request : "+reqURL);
        client.post(reqURL, null, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] response) {
                // called when response HTTP status is "200 OK"
                try{
                    String content = new String(response);
                    DATA.print("--responce in getSpecialist: "+content);
                    DATA.dismissLoaderDefault();

                    try {
                        JSONArray data = new JSONArray(content);
                        DATA.allSpecialities = new ArrayList<SpecialityModel>();
                        SpecialityModel temp = null;

                        if (data.length() == 0) {
                            tvNoData.setVisibility(View.VISIBLE);
                            //showMessageBox(activity, "We are sorry", "Currently no doctors available");
                            lvSpecialistAdapter = new LvSpecialistAdapter(activity);
                            lvDoctors.setAdapter(lvSpecialistAdapter);
                        }else{
                            tvNoData.setVisibility(View.GONE);
                            for (int i = 0; i < data.length(); i++) {
                                temp = new SpecialityModel();
                                JSONObject object = data.getJSONObject(i);
                                temp.specialityId = object.getString("id");
                                temp.specialityName =object.getString("speciality_name");

                                DATA.allSpecialities.add(temp);
                                temp = null;
                            }
                            DATA.print("-- List Size "+DATA.allSpecialities.size());
                            lvSpecialistAdapter = new LvSpecialistAdapter(activity);
                            lvDoctors.setAdapter(lvSpecialistAdapter);
    ...
    public void setupTopBar(){
        if(DialogPatientInfo.patientIdGCM.isEmpty()){
            if(DATA.selectedUserCallId.isEmpty()){
                ...
                //tvVCallViewAddNotes.setVisibility(View.GONE);//remove these 2 line and uncommit tvVCallViewReports clcick to revert
            ...
            public void onClick(View arg0) {
                ...
                if (note_text.isEmpty()) {
                    ...
                    if (checkInternetConnection.isConnectedToInternet()) {
                        ...
                        /*new android.app.AlertDialog.Builder(activity, R.style.CustomAlertDialogTheme)
                                .setTitle("Confirm")
                                ...
                                .setPositiveButton("Save", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        *//*if (activity instanceof AfterCallDialogEmcura) {
                                        ...
                                        SharedPreferences sharedPreferences = getSharedPreferences(DATA.SHARED_PREFS_NAME, Context.MODE_PRIVATE);
                                        SharedPreferences.Editor editor = sharedPreferences.edit();
                                        editor.putString("encounter_notesDurringCall" , note_text);
                                        String existingValue = sharedPreferences.getString("encounter_notesDurringCall", "");
                                        String appendedValue = existingValue + " " + note_text;
                                        editor.putString("encounter_notesDurringCall", appendedValue);
                                        editor.apply();
