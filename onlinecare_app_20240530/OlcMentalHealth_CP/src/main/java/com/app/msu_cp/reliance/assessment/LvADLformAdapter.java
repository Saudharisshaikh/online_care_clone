...
public class LvADLformAdapter extends ArrayAdapter<AdlFormBean> {
	...
	public LvADLformAdapter(Activity activity, ArrayList<AdlFormBean> adlFormBeans) {
		super(activity, R.layout.lv_adl_form_row, adlFormBeans);

		this.activity = activity;
		this.adlFormBeans = adlFormBeans;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			...
			convertView = layoutInflater.inflate(R.layout.lv_adl_form_row, null);
		...
		viewHolder.tvQuestion.setText(adlFormBeans.get(position).question);
		viewHolder.lvOptions.setAdapter(new LvADLoptionAdapter(activity, adlFormBeans.get(position)));
		...
		if(validateFlag){
			int drawableId = adlFormBeans.get(position).isAnswered ? R.drawable.cust_border_white_outline : R.drawable.cust_border_white_outline_red;
