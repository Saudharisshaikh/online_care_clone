	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lvDoctors.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				String msg = "Are you sure? You want to refer "+DATA.selectedApptmntIdsForRefer.size()
						+" appointments to "+DATA.allDoctors.get(arg2).fName+" "+DATA.allDoctors.get(arg2).lName+" ?";


				new AlertDialog.Builder(activity, R.style.CustomAlertDialogTheme)
						.setTitle("Confirm Refer")
						.setMessage(msg)
						.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								if (checkInternetConnection.isConnectedToInternet()) {
									matchreferedSlots(DATA.selectedApptmntIdsForRefer, DATA.allDoctors.get(arg2).id);
								} else {
