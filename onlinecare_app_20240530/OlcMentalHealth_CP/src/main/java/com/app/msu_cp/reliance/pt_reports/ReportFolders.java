package com.app.msu_cp.reliance.pt_reports;
...
public class ReportFolders extends BaseActivity {
	...
	protected void onDestroy() {
		sharedPrefsHelper.save("patient_medical_reports_folders", "");
		super.onDestroy();
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				DATA.selectedReprtFoldrId = ReportFolders.allFolders.get(position).folder_id;
...
//				                       + uploadId + " is: " + progress);
				            ...
				            DATA.print("--online care: The progress of the upload with ID "
				                       + uploadId + " is: " + progress);
				        }
...
//				                       + exception.getLocalizedMessage(), exception);
				            ...
				            DATA.print("--online care: Error in upload with ID: " + uploadId + ". "
				                       + exception.getLocalizedMessage()+ exception);
				        }
...
//				                       + ". Response from server: " + serverResponseMessage);
				            ...
				            DATA.print("--online care: Upload with ID " + uploadId
				                       + " has been completed with HTTP " + serverResponseCode
				                       + ". Response from server: " + serverResponseMessage);
...
//				.show();
	...
	private void getReportsCall() {
		...
		params.put("patient_id", DATA.selectedUserCallId);//patient id
	...
	public void parseReportsData(String content){
		...
		try {
			...
			if(status.equals("success")) {
				...
				ReportFolders.allFolders = new ArrayList<ReportsModel>();
				...
				for (int i = 0; i < foldersArray.length(); i++) {
					...
					ReportFolders.allFolders.add(temp);

					temp = null;
				}

				int vis = ReportFolders.allFolders.isEmpty() ? View.VISIBLE : View.GONE;
				...
				ReportsModel temp1;
				ReportFolders.allReports = new ArrayList<ReportsModel>();
				...
				for (int i = 0; i < reportsArray.length(); i++) {
					...
					ReportFolders.allReports.add(temp1);

					temp1 = null;

				}

				foldersAdapter = new FoldersAdapter(activity, ReportFolders.allFolders);
			...
			DATA.print("--Exception in getReports : "+e);
			e.printStackTrace();
			customToast.showToast(DATA.JSON_ERROR_MSG,0,0);
		}
	}
	...
	public void filterReports(String filterText) {
		...
		ReportFolders.allReportsFiltered = new ArrayList<ReportsModel>();
		ReportsModel temp;

		for(int i = 0; i<ReportFolders.allReports.size(); i++) {

			temp = new ReportsModel();

			if(ReportFolders.allReports.get(i).folder_id.equals(filterText)) {

				DATA.print("item added");

				temp.id = ReportFolders.allReports.get(i).id;
				temp.name = ReportFolders.allReports.get(i).name;
				temp.date = ReportFolders.allReports.get(i).date;
				temp.report_url = ReportFolders.allReports.get(i).report_url;
				temp.report_thumb = ReportFolders.allReports.get(i).report_thumb;

				ReportFolders.allReportsFiltered.add(temp);
