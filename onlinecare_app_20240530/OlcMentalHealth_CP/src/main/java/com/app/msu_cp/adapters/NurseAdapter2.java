...
import com.app.msu_cp.util.MapDialog;
...
public class NurseAdapter2 extends ArrayAdapter<NurseBean2> {
	...
	public NurseAdapter2(AppCompatActivity activity , ArrayList<NurseBean2> nurseBeens) {
...
//		filter(DATA.selectedDrId);
	...
	static class ViewHolder {
		...
		TextView tvNurseName,tvNurseType,tvAssign,tvSendMsg,tvLocation;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			...
			convertView.setTag(R.id.tvLocation, viewHolder.tvLocation);
		}
		else {
		...
		viewHolder.tvLocation.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				new MapDialog(activity).showMapDialog(nurseBeens.get(position).latitude, nurseBeens.get(position).longitude,
						nurseBeens.get(position).first_name+" "+nurseBeens.get(position).last_name,
						nurseBeens.get(position).doctor_category,nurseBeens.get(position).image);
			}
		});
	...
	public void sendMsgSP(String doctorId,String msgText) {
		...
		params.put("to", "specialist");//"doctor"
		...
		client.post(DATA.baseUrl+"/sendmessage",params, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int statusCode, Header[] headers, byte[] errorResponse, Throwable e) {
				...
				try {
					...
					DATA.print("--onfail sendMsg" +content);
