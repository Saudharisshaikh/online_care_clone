...
public class ReportsGridAdapter extends ArrayAdapter<ReportsModel> {
    ...
    {
        super(context, R.layout.reports_grid_row, ReportFolders.allReportsFiltered);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.txtTitle.setText(ReportFolders.allReportsFiltered.get(position).name);
        //holder.imgIcon.setImageResource(position);

        try {
            String extention = ReportFolders.allReportsFiltered.get(position).report_thumb.substring(ReportFolders.allReportsFiltered.get(position).report_thumb.lastIndexOf("."));
            if (extention.equals(".png") || extention.equals(".jpg") || extention.equals(".jpeg")) {
                //UrlImageViewHelper.setUrlDrawable(holder.imgIcon, ReportFolders.allReportsFiltered.get(position).report_thumb, R.drawable.folder_icon);
                //holder.imgIcon.setImageResource(R.drawable.icon_image);
                DATA.loadImageFromURL(ReportFolders.allReportsFiltered.get(position).report_url, R.drawable.ic_placeholder_2, holder.imgIcon);
        ...
        holder.ivDeleteReport.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ((Reports) context).confirmDeleteReportDialog(ReportFolders.allReportsFiltered.get(position).id);
            }
        });
