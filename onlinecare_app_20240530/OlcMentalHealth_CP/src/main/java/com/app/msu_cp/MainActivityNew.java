...
import com.app.msu_cp.adapters.CategoriesAdapter;
import com.app.msu_cp.api.ApiManager;
import com.app.msu_cp.model.CategoriesModel;
import com.app.msu_cp.permission.PermissionsActivity;
...
import com.app.msu_cp.reliance.therapist.ActivityReferedPatients;
...
public class MainActivityNew extends AppCompatActivity implements GloabalSocket.SocketEmitterCallBack{
	...
	TextView tvSearchDoctor,tvMyDoctors,tvMessages,tvTCM,tvComplexCare,tvHomeHealth,
			...
			tvMyPatients,tvReffredPt,tvInstantConnect,tvAppInvite,tvCallHistory,tvSchedulesNew;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvSchedulesNew = findViewById(R.id.tvSchedulesNew);
		...
		tvSchedulesNew.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
	...
	public void showScheduleOptionsDialog(){
		final Dialog dialogSchecduleOp = new Dialog(activity);
		dialogSchecduleOp.requestWindowFeature(Window.FEATURE_NO_TITLE);
		dialogSchecduleOp.setContentView(R.layout.dialog_schedules_home);
		dialogSchecduleOp.setCanceledOnTouchOutside(false);


		ImageView ivClose = dialogSchecduleOp.findViewById(R.id.ivClose);

		TextView tvAppointmentsD = dialogSchecduleOp.findViewById(R.id.tvAppointments);
		TextView tvDrScheduleD = dialogSchecduleOp.findViewById(R.id.tvDrSchedule);
		TextView tvFollowupsD = dialogSchecduleOp.findViewById(R.id.tvFollowups);
		TextView tvNurseAppointmentsD = dialogSchecduleOp.findViewById(R.id.tvNurseAppointments);
		TextView tvSoapRefD = dialogSchecduleOp.findViewById(R.id.tvSoapRef);
		TextView tvPendingNotesD = dialogSchecduleOp.findViewById(R.id.tvPendingNotes);
		TextView tvReffredPtD = dialogSchecduleOp.findViewById(R.id.tvReffredPt);

		TextView tvAppointmentBadgeD = dialogSchecduleOp.findViewById(R.id.tvAppointmentBadge);

		final ArrayList<String> appointments = db.getAllNotifByType(DATA.NOTIF_TYPE_NEW_APPOINTMENT);
		if(tvAppointmentBadgeD!=null){
			if(appointments!= null){
				DATA.print("--appointments size "+appointments.size());
				if(appointments.size() > 0){
					tvAppointmentBadgeD.setText(appointments.size() + "");
					//badge.setTextSize(12);
					tvAppointmentBadgeD.setVisibility(View.VISIBLE);
				}else {
					tvAppointmentBadgeD.setVisibility(View.GONE);
				}
			}else {
				tvAppointmentBadgeD.setVisibility(View.GONE);
			}
		...
		View.OnClickListener onClickListener = new OnClickListener() {
			...
			public void onClick(View v) {
				switch (v.getId()){
					case R.id.ivClose:
						dialogSchecduleOp.dismiss();
						break;
					case R.id.tvAppointments:
						//dialogSchecduleOp.dismiss();
						tvAppointments.performClick();
						break;
					case R.id.tvDrSchedule:
						//dialogSchecduleOp.dismiss();
						tvDrSchedule.performClick();
						break;
					case R.id.tvFollowups:
						//dialogSchecduleOp.dismiss();
						tvFollowups.performClick();
						break;
					case R.id.tvNurseAppointments:
						//dialogSchecduleOp.dismiss();
						tvNurseAppointments.performClick();
						break;
					case R.id.tvSoapRef:
						//dialogSchecduleOp.dismiss();
						tvSoapRef.performClick();
						break;
					case R.id.tvPendingNotes:
						//dialogSchecduleOp.dismiss();
						tvPendingNotes.performClick();
						break;
					case R.id.tvReffredPt:
						//dialogSchecduleOp.dismiss();
						tvReffredPt.performClick();
						break;
				}
		...
		tvAppointmentsD.setOnClickListener(onClickListener);
		tvDrScheduleD.setOnClickListener(onClickListener);
		tvFollowupsD.setOnClickListener(onClickListener);
		tvNurseAppointmentsD.setOnClickListener(onClickListener);
		tvSoapRefD.setOnClickListener(onClickListener);
		tvPendingNotesD.setOnClickListener(onClickListener);
		tvReffredPtD.setOnClickListener(onClickListener);


		dialogSchecduleOp.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
		dialogSchecduleOp.show();
